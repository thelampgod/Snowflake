...
public class ClientTest {
    ...
    void trySendPosition() throws IOException {
        try (Socket s = new Socket("127.0.0.1", 2147)) {
            ...
            out.writeByte(0);
            out.writeUTF(TEST_PUBKEY);
            out.flush();
            System.out.println(in.readUTF());

            PGPSecretKeyRing priv = PGPainless.readKeyRing().secretKeyRing(TEST_PRIVKEY);
            while (true) {
                try {
                    String msg = in.readUTF();
                    System.out.println(msg);

                    String decrypted = EncryptionUtil.decrypt(msg, priv, TEST_PRIVKEY_PASS);
